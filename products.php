...
@media (max-width: 768px) {
  ...
  <a href="gallery.php" class="nav-item nav-link"><strong>Gallery</strong></a>
        ...
        <!-- About Start -->
     <section class="vorc-section py-5">
    <div class="container">
        <h2 class="vorc-title text-center mb-4">Explore VORC Vehicles</h2>
        <p class="vorc-desc text-center mx-auto">
            Welcome to the VORC Motors range of advanced mobility solutions. 
            Our vehicles are purpose-built for the commercial market, offering a powerful 
            blend of electric efficiency, payload capacity, and low running costs. 
            From versatile cargo carriers to dependable passenger vehicles, each model is 
...
const vsData = [
{
    ...
    category:"Passenger",
    models:[
        { image:"1111.png", specs:"" },
        { image:"image__4_-removebg-preview-removebg-preview.png", specs:"" },
        { image:"015.png", specs:"" }
    ]
},
{
    ...
    models:[
        { image:"025.png", specs:"" },
        { image:"Gemini_Generated_Image_kghscakghscakghs-removebg-preview.png", specs:"" }
    ]
},
{
    name:"Premium",
    ...
    models:[
        { image:"002.png", specs:"" },
        { image:"galler3-removebg-preview.png", specs:"#"}
    ]
}
];
...
function renderVS(){
    const wrap = document.getElementById("vsCarousel");
    const navs = wrap.querySelectorAll(".vs-nav");
    wrap.innerHTML="";
    navs.forEach(n=>wrap.appendChild(n));

    const prev = vsIndex===0 ? vsData.length-1 : vsIndex-1;
    const next = vsIndex===vsData.length-1 ? 0 : vsIndex+1;

    wrap.innerHTML += `
        <div class="vs-item vs-prev" onclick="vsPrev()">
            <img src="${vsData[prev].models[0].image}">
        </div>

            <img src="${vsData[vsIndex].models[vsModel].image}">
        </div>

        <div class="vs-item vs-next" onclick="vsNext()">
            <img src="${vsData[next].models[0].image}">
        </div>
    `;

    document.getElementById("vsName").innerText = vsData[vsIndex].name;
    document.getElementById("vsCategory").innerText = vsData[vsIndex].category;
    document.getElementById("vsSpecs").href = vsData[vsIndex].models[vsModel].specs;

    const models = document.getElementById("vsModels");
    models.innerHTML="";
    vsData[vsIndex].models.forEach((m,i)=>{
        models.innerHTML += `
            <div class="vs-model ${i===vsModel?'active':''}" onclick="vsSelect(${i})">
                <div class="vs-model-thumb">
                    <img src="${m.image}">
                </div>
            </div>
        `;
...
function vsPrev(){
    vsIndex = vsIndex===0 ? vsData.length-1 : vsIndex-1;
    vsModel = 0;
    renderVS();
}

function vsNext(){
    vsIndex = vsIndex===vsData.length-1 ? 0 : vsIndex+1;
    vsModel = 0;
    renderVS();
}
...
.container-fluid.banner.pb-5.wow.zoomInDown {
    margin-top: 61px;
}
</style>
...
$(document).ready(function () {
  $("#whee").owlCarousel({
    ...
    /* MOBILE FIXES */
    nav: false,          // ❌ hide arrows on mobile (touch swipe is enough)
    ...
    responsive: {
      0: {
        items: 1,        // ✅ ONE image per row (mobile)
        stagePadding: 0
      },
      768: {
        items: 2,
      ...
      992: {
        ...
        <!-- Car Steps Start -->
       <div class="container-fluid steps py-5" style="background: linear-gradient(120deg, #0a1a3a, #19376d);margin-top: 57px;">
